...
def evaluation_view(request, evaluation_id):
    ...
    # check rights
    if not evaluation.is_user_editor_or_delegate(user) or evaluation.state not in [
        'prepared',
        'editor_approved',
        'approved',
        'in_evaluation',
...
def evaluation_preview(request, evaluation_id):
    ...
    if not (
        ...
        and evaluation.state in ['prepared', 'editor_approved', 'approved', 'in_evaluation', 'evaluated', 'reviewed']
    ):
        raise PermissionDenied
