...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        recipients = set()
        for course in Course.objects.filter(state='in_evaluation', vote_end_date__date__in=check_dates):
        ...
        for recipient in recipients:
            ...
            for course in Course.objects.filter(participants=recipient, state='in_evaluation').exclude(voters=recipient):
                due_courses[course] = (course.vote_end_date.date() - datetime.date.today()).days
