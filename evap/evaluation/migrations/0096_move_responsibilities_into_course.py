...
def move_data_to_course(apps, _schema_editor):
    ...
    for course in Course.objects.all():
        responsibles = [
            ...
            for contribution in Contribution.objects.filter(evaluation=course.evaluation, responsible=True)
        ]
...
def move_data_to_contribution(apps, _schema_editor):
    ...
    for course in Course.objects.all():
        for contribution in Contribution.objects.filter(
            evaluation=course.evaluation, contributor__in=course.responsibles.all()
        ):
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(move_data_to_course, reverse_code=move_data_to_contribution),
    ]
