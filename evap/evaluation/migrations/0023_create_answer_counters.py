...
def create_answer_counters(apps, _schema_editor):
    ...
    for grade_answer in GradeAnswer.objects.all():
        grade_answer_counter, __ = GradeAnswerCounter.objects.get_or_create(
            question=grade_answer.question, contribution=grade_answer.contribution, answer=grade_answer.answer
        )
        grade_answer_counter.count += 1
        grade_answer_counter.save()

    for likert_answer in LikertAnswer.objects.all():
        likert_answer_counter, __ = LikertAnswerCounter.objects.get_or_create(
            question=likert_answer.question, contribution=likert_answer.contribution, answer=likert_answer.answer
        )
