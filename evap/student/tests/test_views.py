...
class TestVoteView(ViewTest):
    ...
    def test_user_cannot_vote_for_themselves(self):
        ...
        for _, contributors, _, _ in response.context['contribution_form_groups']:
            self.assertNotEqual(contributors, [self.contributor1], "Contributor should not see the questionnaire about themselves")

        response = self.get_assert_200(self.url, user=self.voting_user1)
        self.assertTrue(any(self.contributor1 in contributors for _, contributors, _, _ in response.context['contribution_form_groups']),
