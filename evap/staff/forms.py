...
class ContributionFormSet(AtLeastOneFormSet):
    ...
    def handle_moved_contributors(self, data, **kwargs):
        ...
        for i in range(0, total_forms):
            ...
            contributions = Contribution.objects.all().annotate(count=Count('contributors')).filter(count=len(contributors))
            for contributor in contributors:
                contributions = contributions.filter(contributors__pk=contributor)

            if len(contributions) != 1:
                continue
            previous_id = str(contributions[0].id)
