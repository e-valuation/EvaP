...
class UserFormTests(TestCase):
    ...
    def test_participant_changes_respect_unlogged_fields(self):
        user = baker.make(UserProfile)
        semester = baker.make("evaluation.Semester", is_active=True)

        evaluation_with_unlogged = baker.make(Evaluation, course__semester=semester)
        evaluation_with_logged = baker.make(Evaluation, course__semester=semester)

        with patch.object(
            evaluation_with_unlogged.__class__,
            "unlogged_fields",
            new=property(lambda self: ["participants"] if self.pk == evaluation_with_unlogged.pk else []),
        ):
            form_data = get_form_data_from_instance(UserForm, user)
            form_data["evaluations_participating_in"] = [evaluation_with_unlogged.pk, evaluation_with_logged.pk]
            form = UserForm(form_data, instance=user)
            self.assertTrue(form.is_valid())
            form.save()

            self.assertFalse(
                any("participants" in entry.data for entry in evaluation_with_unlogged.related_logentries())
            )

            latest_log = evaluation_with_logged.related_logentries().order_by("id").last()
            self.assertIn("participants", latest_log.data)
            self.assertEqual(latest_log.data["participants"]["add"], [user.pk])
            ...
            latest_log = evaluation_with_logged.related_logentries().order_by("id").last()
            self.assertIn("participants", latest_log.data)
            self.assertEqual(latest_log.data["participants"]["remove"], [user.pk])
