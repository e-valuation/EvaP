...
class EnrollmentImporter(ExcelImporter):
    ...
    def create_test_success_messages(self):
        ...
        msg = format_html(
            ngettext('The import run will create one course/evaluation', 'The import run will create {evaluation} courses/evaluations', len(self.evaluations),).format(evaluation = len(self.evaluations))
        )
        if len(filtered_users) == 0:
            msg += format_html(_(" and no users."))
        else:
            msg += format_html(ngettext(' and one user:', ' and {users} users:', len(filtered_users),).format(users = len(filtered_users)))
...
class UserImporter(ExcelImporter):
    ...
    def save_users_to_db(self):
        ...
        if len(created_users) == 0:
            ...
            msg = format_html(ngettext('Successfully created one user:', 'Successfully created {users} users:', len(created_users),).format(users = len(created_users)))
    ...
    def create_test_success_messages(self):
        ...
        if len(filtered_users) == 0:
            ...
            msg = format_html(ngettext('The import run will create one user:', 'The import run will create {users} users:', len(filtered_users),).format(users = len(filtered_users)))
            msg += create_user_list_html_string_for_message(filtered_users)
        self.success_messages.append(msg)
...
class PersonImporter:
    ...
    def process_participants(self, evaluation, test_run, user_list):
        ...
        if already_related:
            msg = format_html(
                ngettext('The following user is already participant in evaluation {name}:', 'The following {users} are already participants in evaluation {name}:', len(already_related),).format(users = len(already_related), name = evaluation.full_name)
            )
        ...
        if len(users_to_add) == 0:
            ...
            if not test_run:
                ...
                msg = format_html(
                    ngettext('One participant added to the evaluation {name}:', '{users} participants added to the evaluation {name}:', len(users_to_add),).format(name = evaluation.full_name, users = len(users_to_add))
                )
            else:
                msg = format_html(
                    ngettext('One participant would be added to the evaluation {name}:', '{users} participants would be added to the evaluation {name}:', len(users_to_add),).format(name = evaluation.full_name, users = len(users_to_add))
                )
    ...
    def process_contributors(self, evaluation, test_run, user_list):
        ...
        if already_related:
            msg = format_html(
                ngettext('The following user is already contributing to evaluation {name}:', 'The following {users} users are already contributing to evaluation {name}:', len(already_related),).format(users = len(already_related), name = evaluation.full_name)
            )
            msg += create_user_list_html_string_for_message(already_related)
            self.warnings[ImporterWarning.GENERAL].append(msg)
        ...
        if len(users_to_add) == 0:
            ...
            if not test_run:
                ...
                msg = format_html(
                    ngettext('One contributor added to the evaluation {name}:', '{user} contributors added to the evaluation {name}:', len(users_to_add),).format(user = len(users_to_add), name = evaluation.full_name)
                )
            else:
                msg = format_html(
                    ngettext('One contributor would be added to the evaluation {name}:', '{user} contributors would be added to the evaluation {name}:', len(users_to_add),).format(user = len(users_to_add), name = evaluation.full_name)
                )
            msg += create_user_list_html_string_for_message(users_to_add)
