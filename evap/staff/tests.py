...
from evap.evaluation.tools import calculate_average_grades_and_variance
...
class ArchivingTests(WebTest):
    ...
    def test_archiving_does_not_change_results(self):
        ...
        results = {}
        for course in semester.course_set.all():
            results[course] = calculate_average_grades_and_variance(course)

        semester.archive()
        cache.clear()

        for course in semester.course_set.all():
            self.assertTrue(calculate_average_grades_and_variance(course) == results[course])
