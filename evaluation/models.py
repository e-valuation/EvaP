...
class Course(models.Model, metaclass=LocalizeModelBase):
    ...
    def is_in_evaluation_period(self):
        ...
        return now >= self.vote_start_date and now <= self.vote_end_date
    ...
    def days_until_evaluation(self):
        return (self.vote_start_date.day - datetime.datetime.now()).days
    ...
    def update_courses(cls):
        ...
        for course in cls.objects.all():
            try:
                if course.state == "approved" and course.vote_start_date <= now:
                    course.evaluation_begin()
                    course.save()
                    courses_new_in_evaluation.append(course)
                elif course.state == "in_evaluation" and course.vote_end_date < now:
